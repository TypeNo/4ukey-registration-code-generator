...
if(zx==True):
    ...
    for i in range(len(zlist)):
        ...
        if(y<min(i[3] for i in zlist)):
            ...
            a=min(i[3] for i in zlist if i[1]<min(i[3] for i in zlist))
        else:
            x=min(i[0] for i in zlist if i[1]<min(i[3] for i in zlist))
            z=min(i[2] for i in zlist if i[1]<min(i[3] for i in zlist))
            a=min(i[3] for i in zlist if i[1]<min(i[3] for i in zlist))
        q=[x,z]
        r=statistics.mean(q)
        b=min(i[1] for i in rlist)+min(i[3] for i in rlist)
    #r=[min(i[1] for i in zlist),min(i[3]for i in zlist)]
        if(y<b):
            ...
            cv2.line(img,(10,20),(10,a+50),(255,255,255),1)
            cv2.line(img,(10,a+50),(int(r),a+50),(255,255,255),1)
            cv2.line(img,(int(r),a+50),(int(r),a),(255,255,255),1)
